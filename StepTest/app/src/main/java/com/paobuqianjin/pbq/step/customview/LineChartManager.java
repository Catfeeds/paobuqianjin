...
public class LineChartManager {
    private final static String TAG = LineChartManager.class.getSimpleName();
...
//        description.setText("需要展示的内容");
        ...
        xAxis.setTextColor(ContextCompat.getColor(context, R.color.color_f8));
        //保证Y轴从0开始，不然会上移一点
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<StepHisResponse.DataBean.StepListBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            StepHisResponse.DataBean.StepListBean data = dataList.get(i);
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i, (float) data.getStep_number());
            entries.add(entry);
        }
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                ...
                String tradeDate = dataList.get((int) value % dataList.size()).getCreate_day();
                String date = "";
                if (dataList.size() <= 7) {
                    if ((int) value % dataList.size() == 0) {
                        date = DateTimeUtil.formatDateToMD(tradeDate);
                        date = date.replace("-", "月");
                    } else {
                        date = DateTimeUtil.formatDateToD(tradeDate);
                    }
                    return date;
                } else {
                    if ((int) value % dataList.size() == 0) {
                        date = DateTimeUtil.formatDateToMD(tradeDate);
                        date = date.replace("-", "月");
                    } else if ((int) value % dataList.size() % 4 == 0) {
                        date = DateTimeUtil.formatDateToD(tradeDate);
                    } else {
                        date = "";
                    }
        ...
        leftYAxis.setZeroLineColor(ContextCompat.getColor(context, R.color.linechart_back));
        leftYAxis.setZeroLineWidth(1f);
        leftYAxis.setAxisLineWidth(1f);
        leftYAxis.setAxisLineColor(ContextCompat.getColor(context, R.color.linechart_back));
