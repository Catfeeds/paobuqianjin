...
public class SelectSponsorAdapter extends RecyclerView.Adapter<SelectSponsorAdapter.SelectTaskFriendViewHolder> {
    ...
    public SelectSponsorAdapter(Context context, List<UserFriendResponse.DataBeanX.DataBean> data, List<UserFriendResponse.DataBeanX.DataBean> sourceData) {
        super();
    ...
    public void onBindViewHolder(SelectTaskFriendViewHolder holder, int position) {
        if (mData.get(position) instanceof UserFriendResponse.DataBeanX.DataBean) {
            ...
            holder.is_distrubute = mData.get(position).getCusvip();
            if (mData.get(position).getCusvip() == 1) {
            ...
            } else if (mData.get(position).getCusvip() == 0) {
                LocalLog.d(TAG, "非商家VIP");
