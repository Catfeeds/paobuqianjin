...
public class SelectVipAdapter extends RecyclerView.Adapter<SelectVipAdapter.SelectTaskFriendViewHolder> {
    ...
    public SelectVipAdapter(Context context, List<UserFriendResponse.DataBeanX.DataBean> data, List<UserFriendResponse.DataBeanX.DataBean> sourceData) {
        super();
    ...
    public void onBindViewHolder(SelectTaskFriendViewHolder holder, int position) {
        if (mData.get(position) instanceof UserFriendResponse.DataBeanX.DataBean) {
            ...
            } else if (mData.get(position).getVip() == 0) {
                LocalLog.d(TAG, "非VIP");
