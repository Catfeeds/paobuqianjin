...
public class MoonUtils {
    private final static String TAG = MoonUtils.class.getSimpleName();
     ...
     */
    public static void replaceEmoticons(Context context, Editable editable, int start, int count, int id) {
        ...
        while (matcher.find()) {
            ...
            LocalLog.d(TAG, "emot =  " + emot);
            String des = emot.substring(3, emot.length() - 1);
            LocalLog.d(TAG, "des = " + des + "code = " + numToHex8(id));
            if (id != -1 && des.equals(numToHex8(id))) {
                String span = getEmojiStringByUnicode(id);
                LocalLog.d(TAG, "简单验证成功 span = " + span);
    ...
    /*        if (d != null) {
        ...
        LocalLog.d(TAG, editable.toString());
    }
