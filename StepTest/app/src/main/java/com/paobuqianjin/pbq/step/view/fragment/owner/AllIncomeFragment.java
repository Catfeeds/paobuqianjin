...
public class AllIncomeFragment extends BaseFragment {
    ...
    protected void initView(View viewRoot) {
        ...
        allIncomeRecycler.addFooterView(loadMoreView); // 添加为Footer。
        allIncomeRecycler.setLoadMoreView(loadMoreView); // 设置LoadMoreView更新监听。
        if (loadMoreListener != null) {
            allIncomeRecycler.setLoadMoreListener(loadMoreListener); // 加载更多的监听。
        }
    }

    public void loadMoreFinish(boolean dateEmpty, boolean hasMore) {
        if (allIncomeRecycler != null) {
            allIncomeRecycler.loadMoreFinish(dateEmpty, hasMore);
        } else {
