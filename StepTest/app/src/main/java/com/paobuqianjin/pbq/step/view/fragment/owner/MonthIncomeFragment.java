...
public class MonthIncomeFragment extends BaseFragment {
    ...
    protected void initView(View viewRoot) {
        ...
        todayIncomeRecycler = (SwipeMenuRecyclerView) viewRoot.findViewById(R.id.month_income_recycler);
        todayIncomeRecycler.setLayoutManager(layoutManager);
        if (adapter != null) {
            todayIncomeRecycler.setAdapter(adapter);
        }
        this.loadMoreView = new DefineLoadMoreView(getContext());
        todayIncomeRecycler.addFooterView(loadMoreView); // 添加为Footer。
        todayIncomeRecycler.setLoadMoreView(loadMoreView); // 设置LoadMoreView更新监听。
        if (loadMoreListener != null) {
            todayIncomeRecycler.setLoadMoreListener(loadMoreListener); // 加载更多的监听。
        }
    }


    public void loadMoreFinish(boolean dateEmpty, boolean hasMore) {
        if (todayIncomeRecycler != null) {
            todayIncomeRecycler.loadMoreFinish(dateEmpty, hasMore);
        } else {
    ...
    public void scrollTop() {
        if (todayIncomeRecycler != null) {
            todayIncomeRecycler.postDelayed(new Runnable() {
                @Override
                public void run() {
